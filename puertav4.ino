...
int reading;                 //the current reading for the input 
...
Serial.println(irrecv.decodedIRData.decodedRawData, DEC);
    switch(irrecv.decodedIRData.decodedRawData){
      ...
      */
      ...
      reading =  debounce(buttonPin,DEBOUNCELIMIT);
...
irrecv.resume();
  ...
  reading = debounce(buttonPin,DEBOUNCELIMIT);
    ...
    if(maint == LOW){//if in maintement mode 
      ...
      lastButtonState = !reading; // so  the next if statement is true and the door returns to the state it was previously in. 
